...
##dispatch_sync
同步执行，与dispatch_async相反。此方法不能将block加到，mainQueue中，会导致死锁。也不能给Serial Dispatch Queue增加block，也会导致死锁。如下两种方式都将死锁。
...
##dispatch_once
保证在应用程序执行中只执行一次指定的API。常用来声明单例，即使在多线程的环境下，也可以保证安全。
